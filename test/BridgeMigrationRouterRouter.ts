...
describe("BridgeMigrationRouter", async function () {
  ...
  type RADIPairsTokenSymbol = keyof typeof fixture.Pairs.RADI;
  type RADIMigratedPairsTokenSymbol = keyof typeof fixture.Pairs.Migrated.RADI;
  ...
  before(async () => {
    ...
    const router = await ethers.getContractAt("RytellRouter", fixture.Router);

    await fundWAVAX(owner, BigNumber.from(10).pow(28));
    await fundToken(owner, fixture.Tokens.RADI, BigNumber.from(10).pow(25));
    await (
      await getTokenContract(fixture.Tokens.WAVAX)
    ).approve(router.address, ethers.constants.MaxUint256);
    await (
      await getTokenContract(fixture.Tokens.RADI)
    ).approve(router.address, ethers.constants.MaxUint256);
    ...
    for (let tokenSymbol of Object.keys(fixture.Migrators)) {
      ...
      fixture.Migrators[tokenSymbol as MigratorTokenSymbol] =
        bridgeToken.address;
      await bridgeToken
        .connect(owner)
        .mint(
    ...
    for (let tokenSymbol of Object.keys(fixture.Pairs.Migrated.AVAX)) {
      ...
      if (
        fixture.Pairs.Migrated.AVAX[
          tokenSymbol as AVAXMigratedPairsTokenSymbol
        ] !== ""
      ) {
        continue;
      }
      ...
      if (
        (await factory.getPair(fixture.Tokens.WAVAX, tokenAddress)) ==
        ethers.constants.AddressZero
      ) {
        await router
          .connect(owner)
          .addLiquidity(
            fixture.Tokens.WAVAX,
            tokenAddress,
            price,
            price.mul(2),
            price,
      ...
      fixture.Pairs.Migrated.AVAX[tokenSymbol as AVAXMigratedPairsTokenSymbol] =
        await factory.getPair(fixture.Tokens.WAVAX, tokenAddress);
    }

    // if there's no pairs in the migrators, create the pairs and add liquidity for RADI pairs
    for (let tokenSymbol of Object.keys(fixture.Pairs.Migrated.RADI)) {
      let tokenAddress = fixture.Migrators[tokenSymbol as MigratorTokenSymbol];
      if (!tokenAddress) continue;
      if (
        fixture.Pairs.Migrated.RADI[
          tokenSymbol as RADIMigratedPairsTokenSymbol
        ] !== ""
      ) {
        continue;
      }
      ...
      let bn0 = BigNumber.from(fixture.Tokens.RADI);
      let bn1 = BigNumber.from(tokenAddress);

      let [token0, token1] = bn0.gt(bn1)
        ? [bn1.toHexString(), bn0.toHexString()]
        : [bn0.toHexString(), bn1.toHexString()];
      fixture.Pairs.Migrated.RADI[tokenSymbol as RADIMigratedPairsTokenSymbol] =
  ...
  beforeEach(async () => {
    ...
    await WAVAX.connect(account).withdraw(
      await WAVAX.balanceOf(account.address)
    );
    factory = await ethers.getContractFactory("RytellBridgeMigrationRouter");
  ...
  describe("Administration", async function () {
    it("Can be deployed", async function () {
      expect(await migrationRouter.connect(owner).isAdmin(owner.address)).to.be
        .true;
    });

    it("Admin can add admin", async function () {
      await migrationRouter.connect(owner).addAdmin(account.address);
      expect(await migrationRouter.connect(owner).isAdmin(account.address)).to
        .be.true;
    });
    ...
    it("Others can't add admin", async function () {
      expect(await migrationRouter.connect(owner).isAdmin(account.address)).to
        .be.false;
      await expect(migrationRouter.connect(account).addAdmin(account.address))
        .to.be.reverted;
    ...
    it("Others can't remove admin", async function () {
      expect(await migrationRouter.connect(owner).isAdmin(account.address)).to
        .be.false;
      await expect(migrationRouter.connect(account).removeAdmin(owner.address))
        .to.be.reverted;
    ...
    it("Others can't check admin", async function () {
      expect(await migrationRouter.connect(owner).isAdmin(account.address)).to
        .be.false;
      expect(await migrationRouter.connect(account).isAdmin(owner.address)).to
        .be.true;
    });
    ...
    it("Admin can't add migrator incompatible with the token", async function () {
      await expect(
        migrationRouter
          .connect(owner)
          .addMigrator(fixture.Tokens.WAVAX, fixture.Migrators.WBTC)
      ).to.be.reverted;
    });
    ...
    for (let tokenSymbol of Object.keys(fixture.Migrators)) {
      ...
      it(`Others can't add migrator for ${tokenSymbol}`, async function () {
        await expect(
          migrationRouter.connect(account).addMigrator(tokenAddress, migrator)
        ).to.be.reverted;
        expect(
          (await migrationRouter.bridgeMigrator(tokenAddress))
            .toString()
            .toLowerCase()
    ...
    for (let tokenSymbol of Object.keys(fixture.Migrators)) {
      ...
      it(`Admin can add migrator for ${tokenSymbol}`, async function () {
        let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol];
        await migrationRouter
          .connect(owner)
          .addMigrator(tokenAddress, migrator);
        expect(
          (await migrationRouter.bridgeMigrator(tokenAddress))
            .toString()
            .toLowerCase()
        ).to.equal(migrator.toLowerCase());
      });
    }
  });
  ...
  describe("Token Migration", async function () {
    for (let tokenSymbol of Object.keys(fixture.Migrators)) {
      ...
      it(`Any can migrate for token ${tokenSymbol}`, async function () {
        ...
        let tokenAmount = await fundToken(
          account,
          tokenAddress,
        ...
        await tokenContract
          .connect(account)
          .approve(migrationRouter.address, ethers.constants.MaxInt256);
        ...
        let migratedTokenAddress = await migrationRouter.bridgeMigrator(
          tokenAddress
        );
        let migratedToken = await getTokenContract(migratedTokenAddress);
        expect(await migratedToken.balanceOf(account.address)).to.be.equal(
          tokenAmount
        );
      });
    }
  ...
  describe("Liquidity Migration", async function () {
      ...
      for (let tokenSymbol of Object.keys(fixture.Migrators)) {
        ...
        it(`Can migrate liquidity from AVAX-${tokenSymbol}`, async function () {
          ...
          await migrationRouter
            .connect(owner)
            .addMigrator(tokenAddress, migrator);
          let liquidityAmount = await fundLiquidityToken(
            account,
            pairAddress,
            BigNumber.from(10).pow(20)
          );
          await fromPairContract
            .connect(account)
            .approve(migrationRouter.address, ethers.constants.MaxUint256);
          ...
          expect(
            await migratedPair.balanceOf(migrationRouter.address)
          ).to.equal(0);
          expect(
            await fromPairContract.balanceOf(migrationRouter.address)
          ).to.equal(0);
        });
        it(`Can compute accurately chargeback from AVAX-${tokenSymbol}`, async function () {
          ...
          let [chargeBack0, chargeBack1] =
            await migrationRouter.calculateChargeBack(
              pairAddress,
              toPairAddress,
              liquidityAmount
            );
          ...
          await tokenContract
            .connect(account)
            .approve(migrationRouter.address, ethers.constants.MaxUint256);
          expect(await tokenContract.balanceOf(account.address)).to.equal(
            liquidityAmount
          );
          ...
          let expectedChargeBack0 = balance0.gte(previousBalance0)
            ? balance0.sub(previousBalance0)
            : previousBalance0.sub(balance0);
          let expectedChargeBack1 = balance1.gte(previousBalance1)
            ? balance1.sub(previousBalance1)
            : previousBalance1.sub(balance1);
    ...
    describe("RADI", async function () {
      for (let tokenSymbol of Object.keys(fixture.Migrators)) {
        let tokenAddress = fixture.Tokens[tokenSymbol as TokenSymbol];
        if (!((tokenSymbol as AVAXPairsTokenSymbol) in fixture.Pairs.RADI))
          continue;
        if (fixture.TokensWithoutFund.includes(tokenSymbol)) continue;
        it(`Can migrate liquidity from RADI-${tokenSymbol}`, async function () {
          let pairAddress =
            fixture.Pairs.RADI[tokenSymbol as RADIPairsTokenSymbol];
          let toPairAddress =
            fixture.Pairs.Migrated.RADI[tokenSymbol as RADIPairsTokenSymbol];
          let migrator = fixture.Migrators[tokenSymbol as MigratorTokenSymbol];
          await migrationRouter
            .connect(owner)
            .addMigrator(tokenAddress, migrator);
        ...
        it(`Can compute accurately chargeback from RADI-${tokenSymbol}`, async function () {
          let pairAddress =
            fixture.Pairs.AVAX[tokenSymbol as AVAXPairsTokenSymbol];
          let toPairAddress =
            fixture.Pairs.Migrated.AVAX[

...
export async function getTokenContract(
  ...
  return await ethers.getContractAt("IRytellERC20", tokenAddress);
}

export async function getPairContract(pairAddress: string): Promise<Contract> {
  return await ethers.getContractAt("IRytellPair", pairAddress);
}
...
export async function fundToken(
  ...
  if (!(tokenSymbol in fixture.Pairs.AVAX))
    throw `No valid pair for AVAX-${tokenSymbol} required to fund the account with 1INCH from WAVAX`;
  const pairAddress: string = fixture.Pairs.AVAX[tokenSymbol];
  const fundPairContract = await ethers.getContractAt(
    "IRytellPair",
    pairAddress
  );
  let [reserves0, reserves1] = await fundPairContract.getReserves();
  const token0: string = await fundPairContract.token0();
  if (token0 != fixture.Tokens.WAVAX)
  ...
  if (token0 != fixture.Tokens.WAVAX)
    [amountOut0, amountOut1] = [amountOut1, amountOut0];
  expect(
    amountOut0.add(amountOut1),
    "Not enough AVAX used, value is 0 due to rounding issues, use a bigger amountAvax"
  ).to.not.equal(0);
  await fundPairContract
    .connect(account)
    .swap(amountOut0, amountOut1, account.address, []);
  return await tokenContract.balanceOf(account.address);
}

export async function fundLiquidityToken(
  account: SignerWithAddress,
  pairAddress: string,
  amountAvax: BigNumber
): Promise<BigNumber> {
  ...
  let amountToken0 = await fundToken(
    account,
    pairToken0.address,
    amountAvax.div(2)
  );
  let amountToken1 = await fundToken(
    account,
    pairToken1.address,
    amountAvax.div(2)
  );
  ...
  await pairToken0
    .connect(account)
    .transfer(pairContract.address, amountToken0);
  await pairToken1
    .connect(account)
    .transfer(pairContract.address, amountToken1);
